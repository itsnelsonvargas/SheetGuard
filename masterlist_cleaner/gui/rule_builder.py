...
class ColumnRuleEditor(QDialog):
    ...
    def get_rule(self) -> ColumnRule | None:
        ...
        cleaning = [self.cleaning.item(i).text() for i in range(self.cleaning.count()) if self.cleaning.item(i).isSelected()]
        return ColumnRule(
            field_id=fid,
            column=col,
...
class RuleBuilderPanel(QWidget):
    ...
    def __init__(self, parent=None) -> None:
        ...
        self.rule_name_label.setStyleSheet("font-weight: bold; font-size: 13px;")
        
        self.btn_rename = QPushButton("✏️")
        self.btn_rename.setFixedSize(24, 24)
        ...
        self.btn_del_dup = QPushButton("Remove")
        self.btn_add_dup.setObjectName("secondary")
        self.btn_del_dup.setObjectName("secondary")
    ...
    def _add_duplicate_rule(self) -> None:
        ...
        if not ok or not name:
            return
        fields, ok2 = QInputDialog.getText(
            self, "Fields", "Field IDs (comma-separated):"
        )
